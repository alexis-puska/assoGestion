...
export abstract class AbstractAutocompleteControlComponent<Content extends { id?: number }> implements OnInit, ControlValueAccessor {
  ...
  ngOnInit(): void {
    ...
    this.items$ = combineLatest([this.currentItem$, q$]).pipe(
      ...
      map(([resCurrent, resQuery]) => {
        ...
        resQuery = resQuery?.filter(q => !resCurrent.find(c => c?.id === q.id));
